...
from sqlalchemy import create_engine, String, Text, DateTime, JSON, Boolean, Float, Integer
...
class TicketRow(Base):
    ...
    team: Mapped[str] = mapped_column(String, default="General Support")
...
class FeedbackRow(Base):
    """Human-in-the-loop corrections used as continual-learning signals."""
    __tablename__ = "feedback"
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    ticket_id: Mapped[str] = mapped_column(String, index=True)
    analyst: Mapped[str] = mapped_column(String, default="analyst")
...
class LearningExampleRow(Base):
    """Validated examples that can be retrieved as few-shot learning memory."""
    ...
    reason: Mapped[str] = mapped_column(Text, default="")
    source_feedback_id: Mapped[int] = mapped_column(Integer, index=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)


class LearningRunRow(Base):
    """Snapshot of a learning/evaluation cycle."""
...
def log_event(ticket_id, event_type, actor, description, metadata=None):
